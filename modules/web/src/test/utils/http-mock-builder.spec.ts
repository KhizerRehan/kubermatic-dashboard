...
describe('HttpMockBuilder', () => {
  ...
  describe('verifyRequestParams()', () => {
    it('should verify query parameters', () => {
      ...
      const req = httpMock.expectOne('/api/search?query=test&limit=10');

      expect(req.request.url).toContain('/api/search');
  ...
  describe('verifyNoOutstandingRequests()', () => {
    ...
    it('should throw error for outstanding requests', () => {
      httpClient.get('/api/test').subscribe();
      // Not consuming the request at all, so verify() should throw

      expect(() => {
        httpMock.verify();
      }).toThrow();

      // Flush the request so afterEach verify() doesn't fail
      const req = httpMock.expectOne('/api/test');
      req.flush({});
    });
  });
