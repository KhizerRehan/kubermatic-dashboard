...
export class HttpMockBuilder {
   ...
   * ```
   */
  createErrorResponse<T = any>(statusCode: number, errorBody?: T, _message?: string): {message: T; status: number; statusText: string} {
    return {
      message: errorBody,
      status: statusCode,
   ...
   * ```
   */
  createSuccessResponse<T = any>(body: T, _status: number = 200): T {
    return body;
  }
