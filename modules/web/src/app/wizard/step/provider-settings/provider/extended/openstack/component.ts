...
export class OpenstackProviderExtendedComponent extends BaseFormValidator implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.form
      .get(Controls.LoadBalancerClasses)
      .valueChanges.pipe(takeUntil(this._unsubscribe))
      .subscribe(_ => {
        console.log('Grand Parent Value Changed', this.form.value);
      });
      ...
      spec: {
        cloud: {
          openstack: {
            ...
            loadBalancerClasses: this.form.get(Controls.LoadBalancerClasses).value || [],
